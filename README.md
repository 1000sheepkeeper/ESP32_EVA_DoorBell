...
- **门铃从机**（`gatt_server_service_table/`）：安装在门外的门铃端。BLE 栈常驻，
  空闲时不广播；按键后按需开启 BLE 广播，通过 GATT 通知把"门铃事件"发给主机。
...
├── gatt_server_service_table/          # 门铃从机工程（BLE GATT 服务表 + 按需广播）
│   ├── main/                           # 源码：属性表、门铃逻辑、按需广播
...
| 部件 | 硬件 | 职责 |
| --- | --- | --- |
| 门铃从机 | ESP32 开发板 + GPIO16 按键（高电平有效，片内下拉） | 按键按需广播、发门铃通知、本地音频 |
...
└─────────┬──────────┘                                  └─────────┬──────────┘
          │                                                       │
          │ ① BLE 待机，空闲时不广播                                  │ ① 上电: 初始化屏幕/SD/LVGL
          │                                                       │    播放 idle.gif 待机动画
          │ ② 有人按 GPIO16 按键 ──> 开启 BLE 广播                   │ ② 初始化 BLE 主机
          │    并开始广播 (ESP_GATTS_DEMO)                         │
          │                                                       │ ③ 扫描到从机
          │                     ◄────── 连接 ──────                │    建立 BLE 连接
          │                                                       │ ④ 发现服务 0x00FF
          │ ③ 订阅使能 (CCCD 写 0x0100)                            │    订阅特征 0xFF01 通知
          │    发送待处理通知 [0x01, 按键状态, 0, 0] ────►          │ ⑤ 收到通知 (首字节 0x01)
          │                                                       │    播放 knock.gif ×2
          │ ④ 断开/超时 ──> 停止广播并回到待机                       │    + doorbell.wav(I2S)
          │                                                       │ ⑥ 播放结束 → 恢复 idle.gif
          │                                                       │ ⑦ 断开 → 重新扫描连接
          └──────────────────────────────────────────────────────────────┘
```
...
1. **从机待机**：门铃端保持 BLE 栈运行，空闲时停止广播，等待按键事件。
2. **按键广播**：按下 GPIO16 门铃按键，经软件防抖确认后开启按需广播。
...
5. **门铃触发通知**：从机在通知使能后上报待处理的按键事件；
   每次按下 GPIO16 按键都会发送 4 字节通知 `[0x01, 电平, 0x00, 0x00]`。
6. **主机响应**：收到首字节为 `0x01` 的通知后，媒体任务从 SD 卡读取 `doorbell.wav` 经 I2S 播放，
   同时循环播放两轮 `knock.gif`；两者结束后恢复 `idle.gif` 待机动画。
7. **回到待机**：通知发送完成或连接超时后，从机异步关闭连接并停止广播；主机继续扫描并等待下一次连接。
...
- **串口触发**：在串口监视器输入 `1` 或 `p` 即触发门铃音频 + 敲门动画
...
| 通知数据 | `[0x01, GPIO16 电平, 0x00, 0x00]`（首字节 `0x01` = 门铃触发） |
| 射频功率 | 广播与链路均 +9 dBm |
...
2. 主机启动后持续扫描；从机按下 GPIO16 后开始广播，主机日志应出现 `Connected` 与 `Notification register successfully`
3. 按下门铃端 GPIO16 按键 → 主机播放敲门动画与门铃音 → 结束后回到待机动画
4. 观察从机串口：应出现 `Doorbell notification sent to host`，随后异步关闭连接并停止广播
